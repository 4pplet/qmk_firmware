...
Based on HHKB JP matrix code by Jun Wako <wakojun@gmail.com>
...
/*
 * JP variant: 16 rows x 8 cols
 *
 * Uses row extension pins to select between two banks of 8 rows.
 * Row extension pins select which HC4051 is active (active low).
 *
 * Row 0-7:   ROW_EXT_PIN_0 = low,  ROW_EXT_PIN_1 = high (first bank)
 * Row 8-15:  ROW_EXT_PIN_0 = high, ROW_EXT_PIN_1 = low  (second bank)
 */
static inline void jp_key_select_row(uint8_t row) {
    /* Set the 3-bit row address (0-7 within current bank) */
    key_select_row(row);

    /* Select row bank via extension pins */
    key_select_row_ext(row);
}
...
void matrix_init_custom(void) {
    ...
    /* JP-specific: row extension pins */
    gpio_set_pin_output(ROW_EXT_PIN_0);
    gpio_set_pin_output(ROW_EXT_PIN_1);

    /* Set key state as input with pull-up */
    gpio_set_pin_input_high(KEY_STATE_PIN);

    /* Start with enable off (high), prev off, and both row banks disabled */
    key_strobe_low();
    key_prev_off();
...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        ...
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            /*
             * JP needs faster scan due to its twice larger matrix
